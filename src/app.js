...
createApp({
  setup() {
    ...
    function panelStyle(id) { const size = panelSizes.value[id] || {}; return { width: size.width ? `${size.width}px` : undefined, height: size.height ? `${size.height}px` : undefined }; }
    ...
    function readFeedbacks() { try { return JSON.parse(localStorage.getItem(feedbackStorageKey) || "[]"); } catch { return []; } }
    function writeFeedbacks(rows) { localStorage.setItem(feedbackStorageKey, JSON.stringify(rows.slice(0, 50))); }
    function loadFeedbacks() { feedbackItems.value = readFeedbacks().sort((a, b) => String(b.created_at).localeCompare(String(a.created_at))); feedbackError.value = ""; }
    function openFeedbackModal() { feedbackModalOpen.value = true; feedbackForm.value = { displayName: "", content: "" }; feedbackError.value = ""; }
    function closeFeedbackModal() { if (!feedbackSubmitting.value) feedbackModalOpen.value = false; }
    async function submitFeedback() {
      const content = feedbackForm.value.content.trim();
      if (!content) { feedbackError.value = "请先填写反馈内容"; return; }
      feedbackSubmitting.value = true;
      const item = { id: `local-${Date.now()}`, display_name: feedbackForm.value.displayName.trim() || "匿名用户", content, like_count: 0, created_at: new Date().toISOString(), local: true };
      const next = [item, ...readFeedbacks()];
      writeFeedbacks(next); feedbackItems.value = next; feedbackModalOpen.value = false; feedbackNotice.value = "提交成功。"; feedbackSubmitting.value = false;
    }
    function likeFeedback(item) { const next = readFeedbacks().map((row) => row.id === item.id ? { ...row, like_count: numberValue(row.like_count) + 1 } : row); writeFeedbacks(next); feedbackItems.value = next; }
    function toggleAdminMode() { isAdminMode.value = !isAdminMode.value; }
    function deleteFeedback(item) { const next = readFeedbacks().filter((row) => row.id !== item.id); writeFeedbacks(next); feedbackItems.value = next; }
    ...
    onMounted(async () => { await loadInitial(); loadFeedbacks(); await nextTick(); timer = setInterval(() => { if (selectedDate.value === todayText.value && !refreshPaused.value) loadToday(); }, refreshMs); });
